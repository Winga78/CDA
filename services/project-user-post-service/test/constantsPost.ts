import * as dotenv from 'dotenv';
dotenv.config(); 
import { TypeOrmModule } from '@nestjs/typeorm';
import { PostUserModule } from '../src/post-user/post-user.module';
import { AppModule } from '../src/app.module';
import { PostUser } from '../src/post-user/entities/post-user.entity';

export const database  = {
    type :(process.env.DB_TYPE as any)|| "mysql",
    host : process.env.DB_HOST || "host",
    port : process.env.DB_PORT || "3306",
    username : process.env.MYSQL_USER || "db_user",
    password : process.env.MYSQL_PASSWORD || "db_password",
    database : process.env.DB_DATABASE_RELATION || "db_database",
    entities: [PostUser]
}

export const api_project_URL = process.env.VITE_PROJECT_SERVICE_URL||"http://localhost:3002";
export const api_auth_URL = process.env.VITE_AUTH_SERVICE_URL|| "http://localhost:3000";
export const api_chat_URL = process.env.VITE_CHAT_SERVICE_URL|| "http://localhost:3001";

export const imports = [
    TypeOrmModule.forRoot(database),
    AppModule,
    PostUserModule
];